...
import Mxgraph from '@/common/util/mxgraph/mxgraph.vue';
...
export default {
  ...
  beforeCreate () {
    // Para volver a obtener los grupos generales de la institución.
    if (this.$route && this.$route.params && this.$route.params.hasOwnProperty('_id')) {
      const content = this.$route.params;
      let query = '/institucion/' + content.institucion + '/?campos=_id,titulo';
      ...
      .then((res) => {
        if (res) {
          ...
          gruposObtenidos.forEach(grupo => {
            ...
            generalRoles.push(grupo);
          });
          this.$route.params.generalRoles = generalRoles;
          if (Object.keys(this.flowData).length > 0) {
            this.flowData.generalRoles = generalRoles;
          }
        }
      })
  ...
  created () {
    if (this.$route && this.$route.params) {
      console.log('------------------PARAMETROS DEL FLUJO------------------');
      console.log(this.$route.params);
      console.log('------------------------------------');
      this.$store.state.infoFlujo = this.$route.params;
      if (this.$route.params.hasOwnProperty('id')) {
        ...
        this.flowData = {
          ...
          generalRoles: parametros.generalRoles || [] // Pasarle además los roles generales de la institución seleccionada
        };
        ...
        this.flowData = {
          ...
          vigencia: content.vigencia || {},
          generalRoles: content.generalRoles || [] // Pasarle además los roles generales de la institución seleccionada
        };
      }
      /*
